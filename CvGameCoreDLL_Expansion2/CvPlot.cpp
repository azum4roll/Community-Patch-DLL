...
void CvPlot::setPlotType(PlotTypes eNewValue, bool bRecalculate, bool bRebuildGraphics, bool bEraseUnitsIfWater)
{
	...
	TerrainTypes eTerrain = isHills() ? TERRAIN_HILL : (isMountain() ? TERRAIN_MOUNTAIN : NO_TERRAIN);
	if (eTerrain != NO_TERRAIN)
	{
		GC.getMap().ChangeTerrainPlotCount(eTerrain, -1);
		if (getOwner() != NO_PLAYER)
		{
			GET_PLAYER(getOwner()).ChangeTerrainPlotCount(eTerrain, -1);
		}
	}

	m_ePlotType = eNewValue;

	eTerrain = isHills() ? TERRAIN_HILL : (isMountain() ? TERRAIN_MOUNTAIN : NO_TERRAIN);
	if (eTerrain != NO_TERRAIN)
	{
		GC.getMap().ChangeTerrainPlotCount(eTerrain, 1);
		if (getOwner() != NO_PLAYER)
...
void CvPlot::setTerrainType(TerrainTypes eNewValue, bool bRecalculate, bool bRebuildGraphics)
{
	...
	if(eOldValue != eNewValue)
	{
		...
		// Update the map and the plot owner's terrain counts
		if (eOldValue != NO_TERRAIN)
		{
			GC.getMap().ChangeTerrainPlotCount(eOldValue, -1);
			if (getOwner() != NO_PLAYER)
			{
				GET_PLAYER(getOwner()).ChangeTerrainPlotCount(eOldValue, -1);
			}
		}
		if (eNewValue != NO_TERRAIN)
		{
			GC.getMap().ChangeTerrainPlotCount(eNewValue, 1);
			if (getOwner() != NO_PLAYER)
			{
				GET_PLAYER(getOwner()).ChangeTerrainPlotCount(eNewValue, 1);
			}
		}
	}
}
