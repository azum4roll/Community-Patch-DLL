...
CvString CvCityCulture::GetThemingTooltip(BuildingClassTypes eBuildingClass) const
{
	...
	if (iIndex != -1)
	{
		...
		for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++) 
		{
			...
			{
				szRtnValue += "[NEWLINE]";
				szRtnValue += GetLocalizedText("TXT_KEY_CO_TOURISM_THEME_BONUS", pkBuildingInfo->GetThemingYieldBonus(iYield), pkYieldInfo->getIconString(), pkYieldInfo->GetDescription());
			}
		}
	}
	else
	{
		...
		for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++) 
		{
			...
			{
				szRtnValue += "[NEWLINE]";
				szRtnValue += GetLocalizedText("TXT_KEY_CO_TOURISM_THEME_BONUS_NEEDED", pkBuildingInfo->GetThemingYieldBonus(iYield), pkYieldInfo->getIconString(), pkYieldInfo->GetDescription());;
			}
		}
	}
