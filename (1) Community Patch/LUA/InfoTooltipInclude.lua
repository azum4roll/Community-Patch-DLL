...
local function GetInstantYieldString(kYieldInfo, nYield)
	return L("TXT_KEY_PRODUCTION_INSTANT_YIELD", nYield, kYieldInfo.IconString, kYieldInfo.Description);
end
...
local function GetYieldBoostString(kYieldInfo, nYield)
	return L("TXT_KEY_PRODUCTION_BUILDING_YIELD_BOOST", nYield, kYieldInfo.IconString, kYieldInfo.Description);
end
...
local function GetYieldModifierString(kYieldInfo, nYield)
	return L("TXT_KEY_PRODUCTION_BUILDING_YIELD_MODIFIER_GENERIC", nYield, kYieldInfo.IconString, kYieldInfo.Description);
end
...
local function ExtractBuildingYieldTable(tYieldTable, strDatabaseTable, kBuildingInfo, kYieldInfo, strOtherColumn)
	...
	for row in GameInfo[strDatabaseTable]{BuildingType = kBuildingInfo.Type, YieldType = kYieldInfo.Type} do
		...
		tYieldTable[eOtherType][eYield] = row.Yield;
	end
end
...
---@param kBuildingInfo table The building entry
---@param kYieldInfo table The yield entry
local function ExtractBuildingSimpleYieldTable(tYieldTable, strDatabaseTable, kBuildingInfo, kYieldInfo)
	local eYield = kYieldInfo.ID;
	for row in GameInfo[strDatabaseTable]{BuildingType = kBuildingInfo.Type, YieldType = kYieldInfo.Type} do
		tYieldTable[eYield] = row.Yield;
	end
end

---Given a database table with BuildingType, <X>Type, YieldType and Modifier, extract all entries of the given building/yield pair.
---Duplicate rows are overwritten.
---@param tModifierTable table<integer, table<integer, integer?>?> The yield modifier table, could be empty or partially filled
---@param strDatabaseTable string The name of the database table to be extracted
---@param kBuildingInfo table The building entry
---@param kYieldInfo table The yield entry
---@param strOtherColumn string The name of the other column (in the form of <X>Type)
local function ExtractBuildingModifierTable(tModifierTable, strDatabaseTable, kBuildingInfo, kYieldInfo, strOtherColumn)
	local eYield = kYieldInfo.ID;
	for row in GameInfo[strDatabaseTable]{BuildingType = kBuildingInfo.Type, YieldType = kYieldInfo.Type} do
		local eOtherType = GameInfoTypes[row[strOtherColumn]];
		tModifierTable[eOtherType] = tModifierTable[eOtherType] or {};
		tModifierTable[eOtherType][eYield] = row.Modifier;
	end
end
...
---The absolute value of the first parameter is passed into the localization function, and its sign ("+" or "-") is added as the **last** parameter
---@param tTooltipList string[] List of tooltip strings/lines to be added to
---@param strTextKey string Text key to be added
---@param nFirstParameter number First parameter for the text key
---@param ... number|string The rest of the parameters for the text key
...
---Example lines:<br>
---`Factory: +2 [ICON_PRODUCTION] Production +2 [ICON_GOLD] Gold`<br>
---`Astronomy: +3 [ICON_RESEARCH] Science`
---@param tTooltipList string[] List of tooltip strings/lines to be added to
...
---@param tYieldTable table<integer, table<integer, integer?>?> The yield table to be converted into tooltip
...
local function AddTooltipsYieldBoostTable(tTooltipList, strTextKey, tYieldTable, strOtherTypeTable, ...)
	...
	for kOtherTypeInfo in GameInfo[strOtherTypeTable]() do
		local eOtherType = kOtherTypeInfo.ID;
		if tYieldTable[eOtherType] then
			local tBoostStrings = {};
			for eYield = 0, iNumYields - 1 do
				local iYield = tYieldTable[eOtherType][eYield];
				if iYield and iYield ~= 0 then
					table.insert(tBoostStrings, GetYieldBoostString(GameInfo.Yields[eYield], iYield));
				end
			end
			if next(tBoostStrings) then
				AddTooltip(tTooltipList, strTextKey, kOtherTypeInfo.Description, table.concat(tBoostStrings, " "), ...);
			end
		end
	end
end
...
---Example lines:<br>
---`Factory: +2 [ICON_PRODUCTION] Production +2 [ICON_GOLD] Gold in all Cities`<br>
---`Astronomy: +3 [ICON_RESEARCH] Science in all Cities`
---@param tTooltipList string[] List of tooltip strings/lines to be added to
---@param strTextKey string Text key to be added, must have 2 parameters - first is the name of "other type", second is the yield boost
---@param tYieldTable table<integer, table<integer, integer?>?> The yield table to be converted into tooltip
---@param strOtherTypeTable string The name of the database table used as the key of the yield table, e.g. `"Specialists"`
local function AddTooltipsYieldBoostTableGlobal(tTooltipList, strTextKey, tYieldTable, strOtherTypeTable)
	...
	for kOtherTypeInfo in GameInfo[strOtherTypeTable]() do
		...
		if tYieldTable[eOtherType] then
			...
			if next(tBoostStrings) then
				AddTooltipGlobal(tTooltipList, strTextKey, kOtherTypeInfo.Description, table.concat(tBoostStrings, " "));
			end
		end
	end
end
...
---Add a localized yield boost text to the given tooltip table based on the given yield table<br>
---Example lines:<br>
---`+2 [ICON_PRODUCTION] Production +2 [ICON_GOLD] Gold during [ICON_GOLDEN_AGE] Golden Age`<br>
---`+1 [ICON_CULTURE] Culture to Lake Tiles`
---@param tTooltipList string[] List of tooltip strings/lines to be added to
---@param strTextKey string Text key to be added, must have 1+ parameter - the yield string
---@param tYieldTable table<integer, integer?> The yield table to be converted into tooltip
---@param ... number|string The rest of the parameters of strTextKey
local function AddTooltipsSimpleYieldBoostTable(tTooltipList, strTextKey, tYieldTable, ...)
	...
	local tYieldStrings = {};
	for eYield = 0, iNumYields - 1 do
		local iYield = tYieldTable[eYield];
		if iYield and iYield ~= 0 then
			table.insert(tYieldStrings, GetYieldModifierString(GameInfo.Yields[eYield], iYield));
		end
	end
	if next(tYieldStrings) then
		AddTooltip(tTooltipList, strTextKey, table.concat(tYieldStrings, " "), ...);
	end
end

---Add a localized yield boost text, appended with "in all Cities", to the given tooltip table based on the given yield table<br>
---Example lines:<br>
---`+2 [ICON_PRODUCTION] Production +2 [ICON_GOLD] Gold during [ICON_GOLDEN_AGE] Golden Age in all Cities`<br>
---`+1 [ICON_RESEARCH] Science to Luxury Resources in all Cities`
---@param tTooltipList string[] List of tooltip strings/lines to be added to
---@param strTextKey string Text key to be added, must have 1+ parameter - the yield string
---@param tYieldTable table<integer, integer?> The yield table to be converted into tooltip
---@param ... number|string The rest of the parameters of strTextKey
local function AddTooltipsSimpleYieldBoostTableGlobal(tTooltipList, strTextKey, tYieldTable, ...)
	...
	if next(tYieldStrings) then
		AddTooltipGlobal(tTooltipList, strTextKey, table.concat(tYieldStrings, " "), ...);
	end
end

---Add a localized yield modifier text to the given tooltip table based on the given yield modifier table<br>
---Example lines:<br>
---`Windmill: +5% [ICON_PRODUCTION] Production +5% [ICON_GOLD] Gold`<br>
---`Division of Labor: +3% [ICON_RESEARCH] Science`
---@param tTooltipList string[] List of tooltip strings/lines to be added to
...
---@param tYieldModTable table<integer, table<integer, integer?>?> The yield modifier table to be converted into tooltip
...
local function AddTooltipsYieldModifierTable(tTooltipList, strTextKey, tYieldModTable, strOtherTypeTable)
	if not next(tYieldModTable) then
		return;
	end
	for kOtherTypeInfo in GameInfo[strOtherTypeTable]() do
		local eOtherType = kOtherTypeInfo.ID;
		if tYieldModTable[eOtherType] then
			local tModifierStrings = {};
			for eYield = 0, iNumYields - 1 do
				local iModifier = tYieldModTable[eOtherType][eYield];
				if iModifier and iModifier ~= 0 then
					table.insert(tModifierStrings, GetYieldModifierString(GameInfo.Yields[eYield], iModifier));
				end
			end
			if next(tModifierStrings) then
				AddTooltip(tTooltipList, strTextKey, kOtherTypeInfo.Description, table.concat(tModifierStrings, " "));
			end
		end
	end
end
...
---Add a localized yield modifier text to the given tooltip table based on the given yield modifier table<br>
---Example lines:<br>
---`+15% [ICON_PRODUCTION] Production +15% [ICON_GOLD] Gold during [ICON_HAPPINESS_1] "We Love the King Day"`<br>
---`+20% [ICON_RESEARCH] Science during [ICON_GOLDEN_AGE] Golden Age`
---@param tTooltipList string[] List of tooltip strings/lines to be added to
---@param strTextKey string Text key to be added, must have 1 parameter - the yield modifier string
---@param tYieldModTable table<integer, integer?> The yield modifier table to be converted into tooltip
local function AddTooltipsSimpleYieldModifierTable(tTooltipList, strTextKey, tYieldModTable)
	...
	local tModifierStrings = {};
	for eYield = 0, iNumYields - 1 do
		local iModifier = tYieldModTable[eYield];
		if iModifier and iModifier ~= 0 then
			table.insert(tModifierStrings, GetYieldModifierString(GameInfo.Yields[eYield], iModifier));
		end
	end
	if next(tModifierStrings) then
		AddTooltip(tTooltipList, strTextKey, table.concat(tModifierStrings, " "));
	end
end
...
function GetHelpTextForUnit(eUnit, bIncludeRequirementsInfo, pCity)
	...
	if kUnitInfo.UnitEraUpgrade then
		for kEraInfo in GameInfo.Eras() do
			local bAttained = pCity and pActivePlayer:HasReachedEra(kEraInfo.ID);
			if pCity and pActivePlayer:HasReachedEra(kEraInfo.ID) then
	...
	if pCity then
		...
		for strPromotionType, iValue in tAttainedEraPromotions do
...
function GetHelpTextForBuilding(eBuilding, bExcludeName, _, bNoMaintenance, pCity)
	...
	for kAccomplishmentInfo in GameInfo.Accomplishments() do
		...
		if not (pCity and iCompleted >= kAccomplishmentInfo.MaxPossibleCompletions) then
	...
	for eYield = 0, iNumYields - 1 do
		...
		AddTooltipNonZeroSigned(tYieldLines, "TXT_KEY_PRODUCTION_BUILDING_YIELD_MODIFIER", iModifier, kYieldInfo.IconString, kYieldInfo.Description);
		...
		AddTooltipNonZeroSigned(tYieldLines, "TXT_KEY_PRODUCTION_BUILDING_YIELD_MODIFIER", iGlobalMod, kYieldInfo.IconString, kYieldInfo.Description);
	end
	...
	for eYield = 0, iNumYields - 1 do
		...
		ExtractBuildingYieldTable(tBuildingBoosts, "Building_BuildingClassLocalYieldChanges", kBuildingInfo, kYieldInfo, "BuildingClassType");
		ExtractBuildingYieldTable(tBuildingBoostsGlobal, "Building_BuildingClassYieldChanges", kBuildingInfo, kYieldInfo, "BuildingClassType");
		ExtractBuildingModifierTable(tBuildingModifiers, "Building_BuildingClassYieldModifiers", kBuildingInfo, kYieldInfo, "BuildingClassType");
		...
		if eYield == YieldTypes.YIELD_CULTURE and iSpecialistCultureBoost ~= 0 then
			for kSpecialistInfo in GameInfo.Specialists() do
				local eSpecialist = kSpecialistInfo.ID;
	...
	local tBuildingHappiness = {};
	for row in GameInfo.Building_BuildingClassLocalHappiness{BuildingType = kBuildingInfo.Type} do
		tBuildingHappiness[row.BuildingClassType] = row.Happiness;
	end
	for strBuildingClassType, iHappiness in pairs(tBuildingHappiness) do
		AddTooltipNonZero(tLocalAbilityLines, "TXT_KEY_PRODUCTION_BUILDING_BUILDING_HAPPINESS", iHappiness, GameInfo.BuildingClasses[strBuildingClassType].Description);
	end

	local tBuildingHappinessGlobal = {};
	for row in GameInfo.Building_BuildingClassHappiness{BuildingType = kBuildingInfo.Type} do
		tBuildingHappinessGlobal[row.BuildingClassType] = row.Happiness;
	end
	for strBuildingClassType, iHappiness in pairs(tBuildingHappinessGlobal) do
		AddTooltipGlobalNonZero(tGlobalAbilityLines, "TXT_KEY_PRODUCTION_BUILDING_BUILDING_HAPPINESS", iHappiness, GameInfo.BuildingClasses[strBuildingClassType].Description);
	end
	...
	-- Free resources
	local tFreeResources = {};
	for row in GameInfo.Building_ResourceQuantity{BuildingType = kBuildingInfo.Type} do
		tFreeResources[row.ResourceType] = row.Quantity;
	end
	for strResourceType, iQuantity in pairs(tFreeResources) do
		local kResourceInfo = GameInfo.Resources[strResourceType];
		AddTooltipNonZeroSigned(tGlobalAbilityLines, "TXT_KEY_PRODUCTION_BUILDING_FREE_RESOURCE", iQuantity, kResourceInfo.IconString, kResourceInfo.Description);
	end
	...
	for row in GameInfo.Building_FreeSpecUnits{BuildingType = kBuildingInfo.Type} do
		tFreeUnits[row.UnitType] = row.NumUnits;
	end
	for strUnitType, iNumUnits in pairs(tFreeUnits) do
		if iNumUnits > 0 then
			AddTooltip(tLocalAbilityLines, "TXT_KEY_PRODUCTION_BUILDING_FREE_UNIT", iNumUnits, GameInfo.Units[strUnitType].Description);
		end
	end
	...
	for strUnitType, iNumUnits in pairs(tFreeUnits) do
		if iNumUnits > 0 then
			local eUnit = -1;
			local kUnitInfo = GameInfo.Units[strUnitType];
			...
			if kUnitInfo.Found and pActivePlayer:MayNotAnnex() then
				if not eMerchantOfVeniceUnit then
					for kUnitInfoLoop in GameInfo.Units{CanBuyCityState = false} do
						-- Only get the first like in DLL. We already assumed there's only one unit.
						eMerchantOfVeniceUnit = kUnitInfoLoop;
						break;
					end
				end
			...
			if eUnit ~= -1 then
				AddTooltip(tLocalAbilityLines, "TXT_KEY_PRODUCTION_BUILDING_FREE_UNIT", iNumUnits, GameInfo.Units[eUnit].Description);
			end
		end
	end
	...
	if iMaxPlayerInstance ~= -1 then
		local strExtraInstance = pCity and L("TXT_KEY_PRODUCTION_BUILDING_EXTRA_PLAYER_INSTANCE_SUFFIX", iCurrentExtraInstances) or "";
	...
	for eYield = 0, iNumYields - 1 do
		...
		local eTech = GameInfoTypes[strTechType];
		if strTechType and iTourism > 0 and eYield == YieldTypes.YIELD_TOURISM and not (pCity and pActivePlayer:HasTech(eTech)) then
			tTechBoosts[eTech] = tTechBoosts[eTech] or {};
	...
	for kAccomplishmentInfo in GameInfo.Accomplishments() do
		...
		local strDomainType;
		local iDomainXP = 0;
		local strUnitCombatType;
		local iUnitProductionMod = 0;
		for row in GameInfo.Building_BonusFromAccomplishments{BuildingType = kBuildingInfo.Type, AccomplishmentType = kAccomplishmentInfo.Type} do
			iHappiness = row.Happiness;
			strDomainType = row.DomainType;
			iDomainXP = row.DomainXP;
			strUnitCombatType = row.UnitCombatType;
			iUnitProductionMod = row.UnitProductionModifier;
		end
		AddTooltipNonZeroSigned(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_HAPPINESS_FROM_ACCOMPLISHMENT", iHappiness, kAccomplishmentInfo.Description);
		if strDomainType then
			AddTooltipNonZeroSigned(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_XP_FOR_DOMAIN_FROM_ACCOMPLISHMENT", iDomainXP,
			kAccomplishmentInfo.Description, GameInfo.Domains[strDomainType].Description);
		end
		if strUnitCombatType then
			AddTooltipNonZeroSigned(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_UNIT_COMBAT_PRODUCTION_MODIFIER_FROM_ACCOMPLISHMENT", iUnitProductionMod,
			kAccomplishmentInfo.Description, GameInfo.UnitCombatInfos[strUnitCombatType].Description);
		end
	end

	for kOtherBuildingInfo in GameInfo.Buildings() do
		local iHappiness = 0;
		for row in GameInfo.Building_BuildingClassLocalHappiness{BuildingType = kOtherBuildingInfo.Type, BuildingClassType = kBuildingClassInfo.Type} do
			iHappiness = row.Happiness;
		end
		AddTooltipNonZero(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_HAPPINESS_FROM_BUILDING", iHappiness, kOtherBuildingInfo.Description);

		local iHappinessGlobal = 0;
		for row in GameInfo.Building_BuildingClassHappiness{BuildingType = kOtherBuildingInfo.Type, BuildingClassType = kBuildingClassInfo.Type} do
			iHappinessGlobal = row.Happiness;
		end
		AddTooltipNonZero(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_HAPPINESS_FROM_BUILDING_GLOBAL", iHappinessGlobal, kOtherBuildingInfo.Description);
	end
	...
	if pCity and MOD_BALANCE_CORE_BUILDING_INVESTMENTS and pCity:GetNumRealBuilding(eBuilding) <= 0 and pCity:GetNumFreeBuilding(eBuilding) <= 0 then
		...
		local strInvestRules = L("TXT_KEY_PRODUCTION_BUILDING_INVESTMENT", iAmount, iWonderAmount);
		table.insert(tLines, strInvestRules);
		local iCostModifier = pCity:GetWorldWonderCost(eBuilding);
		if iCostModifier > 0 then
			local strWonderCostMod = L("TXT_KEY_PRODUCTION_BUILDING_WONDER_COST_INCREASE_METRIC", iCostModifier);
