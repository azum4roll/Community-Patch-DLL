...
function GetHelpTextForBuilding(eBuilding, bExcludeName, _, bNoMaintenance, pCity)
	...
	if not pCity then
		...
		for row in GameInfo.Building_LocalFeatureOrs{BuildingType = kBuildingInfo.Type} do
			table.insert(tReqFeatures, GameInfo.Features[row.FeatureType].Description);
		end
		...
		if next(tBuildings) then
			AddTooltip(tReqLines, "TXT_KEY_PRODUCTION_REQUIRED_BUILDINGS_GLOBAL", table.concat(tBuildings, ", "));
		end
