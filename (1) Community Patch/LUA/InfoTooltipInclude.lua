...
---@param tYieldTable table<integer, table<integer, integer?>?> The yield/modifier table to be converted into tooltip. If strOtherTypeTable is nil, the table is tYieldTable[1] and other entries are ignored.
...
local function AddTooltipsYieldTableGeneric(tTooltipList, strTextKey, tYieldTable, strOtherTypeTable, bModifier, bGlobal, ...)
	...
	if strOtherTypeTable then
			...
			if tYieldTable[eOtherType] then
				...
				for eYield = 0, iNumYields - 1 do
					...
					if iYield and iYield ~= 0 then
						table.insert(tBoostStrings, GetStringFunc(GameInfo.Yields[eYield], iYield));
					end
				end
		...
		local tYieldStrings = {};
		for eYield = 0, iNumYields - 1 do
			local iYield = tYieldTable[1][eYield];
			if iYield and iYield ~= 0 then
				table.insert(tYieldStrings, GetStringFunc(GameInfo.Yields[eYield], iYield));
			end
		end
		if next(tYieldStrings) then
			AddTooltipFunc(tTooltipList, strTextKey, table.concat(tYieldStrings, " "), ...);
		end
	end
end
...
function GetHelpTextForUnit(eUnit, bIncludeRequirementsInfo, pCity)
	...
	local stop = os.clock();
	print("Unit: " .. L(kUnitInfo.Description) .. ", Time: " .. stop - start);
...
function GetHelpTextForBuilding(eBuilding, bExcludeName, _, bNoMaintenance, pCity)
	...
	for row in GameInfo.Building_SpecificGreatPersonRateModifier{BuildingType = kBuildingInfo.Type} do
		...
		AddTooltipPositive(tLocalAbilityLines, "TXT_KEY_PRODUCTION_BUILDING_GPP_MODIFIER", row.Modifier, kGreatPersonInfo.IconString, kGreatPersonInfo.Description);
	end
	...
	for kOtherBuildingInfo in GameInfo.Buildings() do
		...
		AddTooltipNonZeroSigned(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_HAPPINESS_FROM_BUILDING", iHappiness, kOtherBuildingInfo.Description);
		...
		AddTooltipNonZeroSigned(tBoostLines, "TXT_KEY_PRODUCTION_BUILDING_HAPPINESS_FROM_BUILDING_GLOBAL", iHappinessGlobal, kOtherBuildingInfo.Description);
	end
...
local gStop = os.clock();
print("Initialization time: ".. gStop - gStart);
